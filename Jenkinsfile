pipeline {
        ...
        stage('Clone Repository') {
            steps {
                git branch: 'working_main',
                    url: 'https://github.com/DebarghyaSikder/EGov-Grievance-backend.git'
            }
        }

        stage('Build Microservices (Maven)') {
            steps {
                dir('egov-management-microservices') {
                    bat '''
                    for /d %%d in (*) do (
                        if exist "%%d\\pom.xml" (
                            echo ====================================
    ...
    post {
        success {
            echo 'E Governance grievance redressal system built and deployed successfully!'
        }
        failure {
